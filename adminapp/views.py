...
from .forms import Task_Form, StudentForm, Contact_Form
...
def ContactManager(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect('adminapp:ContactManager')  # Redirect to the correct view name after saving
    else:
        form = Contact_Form()

    # Fetch all contacts from the database to display
